...
import com.experiment_P.p6.User;
...
public class Test {
    ...
    private static String getCollegeName(List<Student> students, int sNumber) {
        return  students.stream()
                .filter(student -> sNumber == student.getNumber())
                .findFirst()
    ...
    private static void printCollegeName(List<Student> students, int sNumber, int tNumber) {
        students.stream()
                ...
                .findFirst()
                .map(Student::getTeacher)
                .filter(teacher -> tNumber == teacher.getNumber())
                .ifPresentOrElse(teacher -> System.out.println(teacher.getCollege().getName()),
